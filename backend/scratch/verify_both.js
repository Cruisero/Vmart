...
try {
  const p2 = formatPrivateKey(configPriv + "=");
  const keyObj2 = crypto.createPrivateKey({ key: p2, format: 'pem' });
  console.log('Config Private Key + "=" is valid!');
} catch (err) {
  console.log('Config Private Key + "=" is invalid:', err.message);
}
