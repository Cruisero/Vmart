const crypto = require('crypto');

const configPriv = "";

function formatPrivateKeyPKCS1(rawKey) {
  if (rawKey.includes('BEGIN')) return rawKey;
  return `-----BEGIN RSA PRIVATE KEY-----\n${rawKey.match(/.{1,64}/g).join('\n')}\n-----END RSA PRIVATE KEY-----`;
}

try {
  const p = formatPrivateKeyPKCS1(configPriv);
  const keyObj = crypto.createPrivateKey({ key: p, format: 'pem' });
  console.log('Config Private Key loaded as PKCS#1 (RSA PRIVATE KEY) successfully!');
} catch (err) {
  console.log('Config Private Key failed as PKCS#1 (RSA PRIVATE KEY):', err.message);
}

try {
  const p = formatPrivateKeyPKCS1(configPriv + "=");
  const keyObj = crypto.createPrivateKey({ key: p, format: 'pem' });
  console.log('Config Private Key + "=" loaded as PKCS#1 successfully!');
} catch (err) {
  console.log('Config Private Key + "=" failed as PKCS#1:', err.message);
}

try {
  const p = formatPrivateKeyPKCS1(configPriv + "==");
  const keyObj = crypto.createPrivateKey({ key: p, format: 'pem' });
  console.log('Config Private Key + "==" loaded as PKCS#1 successfully!');
} catch (err) {
  console.log('Config Private Key + "==" failed as PKCS#1:', err.message);
}
