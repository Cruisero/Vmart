...
export default function TenantSettings() {
    ...
    // Basic Info表单
    const [shopForm, setShopForm] = useState({ name: '', notice: '', skin: 'fresh', logo: '', favicon: '', agentEnabled: false, language: 'zh', currency: 'CNY' })
    // 特色卡片
...
- We reserve the right to restrict purchase access for accounts that maliciously or repeatedly request refunds.`)
    ...
    const tabs = [
        ...
        { key: 'order', icon: '📋', label: L('inline.orders.19d0b2a') },
    ...
    return (
        <div className="ts-page">
            ...
            {activeTab === 'order' && (
                <div className="ts-section">
                    <div className="ts-section-desc">
                        <h3>{L('inline.order.settings.f9ade7b')}</h3>
                        <p>{L('inline.configure.stock.calculation.and.order.timeout.rules.8ee9656')}</p>
                    </div>
                    <div className="ts-form">
                        <div className="ts-form-group">
                            ...
                            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                                ...
                                >
                                    ...
                                    <div>
                                        <div style={{ fontSize: '0.9rem', fontWeight: 600, color: 'var(--text-primary)' }}>🔄 {L('inline.auto.53a3bc0')}</div>
                                        <div style={{ fontSize: '0.75rem', color: 'var(--text-muted)', marginTop: 2 }}>{L('inline.stock.equals.available.card.count.and.is.deducted.after.deli.0efda59')}</div>
                                    </div>
                                </div>
                                <div
                                ...
                                >
                                    ...
                                    <div>
                                        <div style={{ fontSize: '0.9rem', fontWeight: 600, color: 'var(--text-primary)' }}>✏️ {L('inline.manual.843ab14')}</div>
                                        <div style={{ fontSize: '0.75rem', color: 'var(--text-muted)', marginTop: 2 }}>{L('inline.enter.stock.manually.without.linking.it.to.card.count.9693c33')}</div>
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="ts-form-group">
                            <label>{L('inline.order.timeout.982bde5')}</label>
                            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                                <input
                                    ...
                                    style={{ width: 100 }}
                                />
                                <span style={{ fontSize: '0.85rem', color: 'var(--text-muted)' }}>{L('inline.minutes.96cd1c2')}</span>
                            </div>
                            <span className="ts-hint">{L('inline.unpaid.orders.are.cancelled.automatically.after.timeout.897dd2e')}</span>
                        </div>

                        <button type="button" className="ts-btn-primary" onClick={handlePaySave}>
                            {L('inline.save.order.settings.1cc7703')}
                        </button>
                    </div>
                </div>
            )}
