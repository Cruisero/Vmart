...
export default function FreshUserCenter() {
    ...
    if (!isAuthenticated) return (
        <div className="fuc-page">
            <div className="fuc-not-logged">
                ...
                <h2>请先登录</h2>
                <p>登录后可查看个人信息和订单</p>
                <Link to="/login" className="fuc-submit-btn" style={{ display: 'inline-block', textDecoration: 'none', textAlign: 'center' }}>
                    去登录
                </Link>
            </div>
        </div>
    ...
    const handleLogout = () => { logout(); toast.success('已退出登录'); navigate('/') }

    const tabs = [
        { path: '/user/orders', label: '我的订单', icon: FiPackage },
        { path: '/user/password', label: '修改密码', icon: FiLock },
    ]
