...
export default function ZenResetPassword() {
    const L = useBuyerL()
            ...
            <div className="za-page">
                <div className="za-card" style={{ textAlign: 'center' }}>
                    <div className="za-header">
                        <div className="za-eyebrow">LINK EXPIRED</div>
                        <h1 className="za-title">{L('链接无效', 'Invalid Link')}</h1>
                        <p className="za-sub" style={{ marginTop: 8 }}>{L('重置密码链接无效或已过期', 'The password reset link is invalid or has expired')}</p>
                    </div>

                    <Link to={`${prefix}/forgot-password`} className="za-guest">{L('重新请求重置链接', 'Request Another Link')}</Link>
                </div>
            </div>
        )
    }
    ...
    if (success) {
        return (
            <div className="za-page">
                <div className="za-card" style={{ textAlign: 'center' }}>
                    <div className="za-header">
                        <div className="za-eyebrow">SUCCESS</div>
                        <h1 className="za-title">{L('密码重置成功', 'Password Reset Successful')}</h1>
                        <p className="za-sub" style={{ marginTop: 8 }}>{L('您的密码已更新，请使用新密码登录', 'Your password has been updated. Please log in with your new password.')}</p>
                    </div>
    ...
    return (
        <div className="za-page">
            <div className="za-card">
                <div className="za-header">
                    <div className="za-eyebrow">RESET PASSWORD</div>
                    <h1 className="za-title">{L('设置新密码', 'Set New Password')}</h1>
                    <p className="za-sub">{L('请输入您的新密码', 'Please enter your new password below')}</p>
                </div>

                <form className="za-form" onSubmit={handleSubmit}>
                    <div className="za-field">
                        <label className="za-label">{L('新密码', 'New Password')}</label>
                    ...
                    <div className="za-field">
                        <label className="za-label">{L('确认密码', 'Confirm Password')}</label>
                        <div className="za-input-wrap">
                            <FiLock className="za-input-icon" size={15} />
                            <input
                                ...
                                autoComplete="new-password"
                            />
                            <button type="button" className="za-toggle-pw" onClick={() => setShowConfirmPw(!showConfirmPw)}>
                    ...
                    <button type="submit" className="za-submit" disabled={loading}>
                        {loading ? <span className="za-spinner" /> : L('确认重置', 'Reset Password')}
                    </button>
                </form>
