...
export default function ZenOrderResult() {
    ...
    const statusConfig = {
        pending:   { label: L('order.pending'),   icon: FiClock,        bg: '#FEF3C7', iconBg: '#FDE68A', iconColor: '#D97706', border: '#FCD34D' },
        paid:      { label: L('order.paid'),      icon: FiCheck,        bg: '#EFF6FF', iconBg: '#DBEAFE', iconColor: '#2563EB', border: '#BFDBFE' },
        completed: { label: L('order.completed'), icon: FiCheck,        bg: '#ECFDF5', iconBg: '#D1FAE5', iconColor: '#059669', border: '#A7F3D0' },
        cancelled: { label: L('order.cancelled'), icon: FiAlertCircle,  bg: '#FEF2F2', iconBg: '#FEE2E2', iconColor: '#DC2626', border: '#FECACA' },
        refunded:  { label: L('order.refunded'),  icon: FiRefreshCw,    bg: '#F5F3FF', iconBg: '#EDE9FE', iconColor: '#B91C1C', border: '#DDD6FE' },
    }
    ...
    const formatCountdown = (s) => {
        if (s === null || s <= 0) return L('order.expired')
        ...
        const iv = setInterval(async () => {
            try {
                ...
                if (data.orderStatus === 'paid' || data.orderStatus === 'completed') {
                    setQrCodeUrl(null); toast.success(L('common.success')); window.location.reload()
                }
            } catch {}
        }, 3000)
    ...
    const handlePayment = async () => {
        ...
        try {
            ...
            if (data.paymentType === 'usdt' || data.paymentType === 'bsc_usdt') {
                ...
                toast.success(L('order.scanToPay'))
            } else if (data.payUrl) {
                window.location.href = data.payUrl
            } else {
                toast.error(data.error || L('common.failed'))
            }
        } catch {
            toast.error(L('common.networkError'))
        } finally {
            setPaying(false)
        }
    }
    ...
    const cancelPayment = async () => {
        try {
            ...
            const data = await res.json()
            if (res.ok) {
                toast.success(L('order.cancelSuccess'))
    ...
    if (!order) return (
        <div className="for-page">
            <div className="for-empty">
                ...
                <h2>{L('orderQuery.notFound')}</h2>
                <p>{orderNo}</p>
                <Link to={`${prefix}/order-query`} className="for-btn-outline">{L('orderQuery.query')}</Link>
            </div>
        </div>
    )
        ...
        <div className="for-page">
                ...
                <div className="for-card">
                    {usdtPayment ? (
                        <div className="for-usdt">
                            ...
                            <div className="for-usdt-info">Rate: 1 USDT = {formatPrice(usdtPayment.exchangeRate, currency)} · {formatPrice(order.totalAmount, currency)}</div>
                            ...
                            {countdown !== null && (
                                <div className={`for-countdown${countdown <= 60 ? ' urgent' : ''}`}>
                                    <FiClock size={14} /> {L('order.countdown')}: <strong>{formatCountdown(countdown)}</strong>
                                </div>
                            )}
                            <button className="for-btn-ghost" onClick={cancelPayment}>{L('order.cancelOrder')}</button>
                        </div>
                    ) : qrCodeUrl ? (
                        <div className="for-qr-section">
                            <div className="for-qr-title">{L('order.scanToPay')}</div>
                            ...
                            <div className="for-qr-amount">{formatPrice(order.totalAmount, currency)}</div>
                            <div className="for-qr-hint">{L('order.scanToPay')}</div>
                            {countdown !== null && (
                                <div className={`for-countdown${countdown <= 60 ? ' urgent' : ''}`}>
                                    <FiClock size={14} /> {L('order.countdown')}: <strong>{formatCountdown(countdown)}</strong>
                                </div>
                        ...
                        <div className="for-pending-default">
                            <div className="for-pending-notice">
                                ...
                                <div>
                                    <div className="for-pn-title">{L('order.pending')}</div>
                                    <div className="for-pn-sub">{L('order.countdown')}</div>
                                </div>
                            </div>
                            <button className="for-pay-btn" onClick={handlePayment} disabled={paying}>
                                {paying ? L('checkout.submitting') : `${L('checkout.submit')} ${formatPrice(order.totalAmount, currency)}`}
                            </button>
                        </div>
                    )}
                </div>
            ...
            {(order.status === 'completed' || order.status === 'paid') && order.cards.length > 0 && (
                <div className="for-card">
                    <div className="for-card-header">
                        <div className="for-card-title">🎁 {L('order.cardKeys')}</div>
                        <button className="for-copy-all" onClick={() => copy(order.cards.map(c => c.content).join('\n'))}>
                            <FiCopy size={12} /> {L('order.copyAll')}
                        </button>
                    </div>
                    <div className="for-cards-list">
                        {order.cards.map((card, i) => (
                            <div key={card.id} className="for-card-item">
                                ...
                                <button className="for-copy-icon" onClick={() => copy(card.content)} title={L('order.copy')}>
                                    <FiCopy size={13} />
                                </button>
                            </div>
                        ))}
                    ...
                    <div className="for-cards-footer">
                        <span><strong>{order.cards.length}</strong> {L('order.cardKeys')}</span>
                    </div>
                </div>
            )}
            ...
            {order.status === 'cancelled' && (
                <div className="for-card for-status-notice cancelled">
                    ...
                    <div>
                        <div className="for-pn-title">{L('order.cancelled')}</div>
                    </div>
                </div>
            )}
            ...
            {order.status === 'refunded' && (
                <div className="for-card for-status-notice refunded">
                    <FiRefreshCw size={20} />
                    <div>
                        <div className="for-pn-title">{L('order.refunded')}</div>
                    </div>
                </div>
            )}
            ...
            {/* ── Product info ── */}
            <div className="for-card">
                <div className="for-card-title" style={{ marginBottom: 16 }}>{L('checkout.productInfo')}</div>
                <div className="for-product-row">
                    ...
                    <div className="for-product-info">
                        ...
                        <div className="for-product-qty">{L('products.quantity')}: {order.quantity}</div>
                    </div>
                    <div className="for-product-price">{formatPrice(order.totalAmount, currency)}</div>
                </div>
            </div>
            ...
            {(order.status === 'completed' || order.status === 'paid') && order.cards.length === 0 && (
                <div className="for-card for-cards-pending">
                    ...
                    <div>
                        <div className="for-pn-title" style={{ color: '#065F46' }}>{L('order.cardKeys')}</div>
                    </div>
                </div>
            )}
            ...
            {/* ── Order details ── */}
            <div className="for-card">
                <div className="for-card-title" style={{ marginBottom: 16 }}>{L('order.title')}</div>
                <div className="for-details">
                    <div className="for-detail-row">
                        <span className="for-detail-label">{L('order.status')}</span>
                    ...
                    {[
                        [L('order.orderNo'), order.orderNo],
                        [L('checkout.email'), order.email],
                        [L('order.paymentMethod'), order.paymentMethod],
                        [L('order.createdAt'), order.createdAt],
                        ...(order.paidAt ? [[L('order.paidAt'), order.paidAt]] : []),
                        [L('order.amount'), formatPrice(order.totalAmount, currency)],
                    ].map(([label, val], i, arr) => (
