...
export default function TenantSettings() {
    ...
    const { token: mToken, updateShop, merchant } = useMerchantStore()
    const L = useAdminL()
    const adminLang = useAdminPrefsStore((s) => s.language)
    const [tenant, setTenant] = useState(null)
    const shop = tenant?.shop || null
    ...
    // Basic Info表单
    const [shopForm, setShopForm] = useState({ name: '', notice: '', skin: 'fresh', logo: '', favicon: '', bookmarkTitle: '', seoDescription: '', seoKeywords: '', agentEnabled: false, language: 'zh', currency: 'CNY', contactEnabled: false, contactTelegram: '', contactWhatsapp: '', contactEmail: '', contactQq: '', contactWechat: '', contactLinks: [] })
    // 特色卡片
...
- We reserve the right to restrict purchase access for accounts that maliciously or repeatedly request refunds.`)
    ...
    const [payForm, setPayForm] = useState({
        ...
        notifyOrderPaid: true, notifyShipRemind: true, notifyNewTicket: true, notifyNewUser: false, notifyStockAlert: true, notifyOrderCancel: false, notifyRefund: true, notifyTicketClosed: true, notifyEmail: '',
    ...
    useEffect(() => {
        ...
        Promise.all([
            ...
            fetch('/api/admin/plan-limits', { headers: { Authorization: `Bearer ${token}` } }).then(r => r.json()).catch(() => null),
            fetch('/api/admin/settings', { headers: { Authorization: `Bearer ${token}` } }).then(r => r.json()).catch(() => null)
        ]).then(([tenantData, settingsData, limitsData, adminSettingsData]) => {
            const t = tenantData.tenant
            setTenant(t)
            setTenantSettings(settingsData.settings)
            
            const tenantShop = t?.shop
            let shopSettings = {}
            if (tenantShop?.settings) {
                try { shopSettings = JSON.parse(tenantShop.settings) } catch {}
            }
            ...
            if (tenantShop && updateShop) updateShop(tenantShop)
            if (t) {
                let parsedLinks = []
                if (Array.isArray(sysSettings.contactLinks)) {
                    parsedLinks = sysSettings.contactLinks
                } else {
                    if (sysSettings.contactTelegram) parsedLinks.push({ id: 'tg', type: 'telegram', value: sysSettings.contactTelegram, label: '' })
                    if (sysSettings.contactWhatsapp) parsedLinks.push({ id: 'wa', type: 'whatsapp', value: sysSettings.contactWhatsapp, label: '' })
                    if (sysSettings.contactEmail || sysSettings.contactGmail) parsedLinks.push({ id: 'mail', type: 'email', value: sysSettings.contactEmail || sysSettings.contactGmail, label: '' })
                    if (sysSettings.contactQq) parsedLinks.push({ id: 'qq', type: 'qq', value: sysSettings.contactQq, label: '' })
                    if (sysSettings.contactWechat) parsedLinks.push({ id: 'wx', type: 'wechat', value: sysSettings.contactWechat, label: '' })
                }
                ...
                setShopForm({
                    ...
                    contactEnabled: sysSettings.contactEnabled === true || sysSettings.contactEnabled === 'true',
                    contactTelegram: sysSettings.contactTelegram || '',
                    contactWhatsapp: sysSettings.contactWhatsapp || '',
                    contactEmail: sysSettings.contactEmail || sysSettings.contactGmail || '',
                    contactQq: sysSettings.contactQq || '',
                    contactWechat: sysSettings.contactWechat || '',
            ...
            if (settingsData.settings) {
                ...
                setPayForm({
                    ...
                    notifyTicketClosed: payConfig.notify_ticket_closed !== false,
        ...
        try {
            ...
            const r = await fetch(`${API}/tenant/setup`, {
                ...
                body: JSON.stringify({
                    ...
                    favicon: shopForm.favicon,
                    bookmarkTitle: shopForm.bookmarkTitle,
                    seoDescription: shopForm.seoDescription,
                    seoKeywords: shopForm.seoKeywords
                })
            })
            ...
            if (r.ok) {
                ...
                if (d.tenant?.shop && updateShop) updateShop(d.tenant.shop)
                ...
                await fetch('/api/admin/settings', {
                    ...
                    body: JSON.stringify({
                        ...
                        contactEnabled: !!shopForm.contactEnabled,
                        contactTelegram: (shopForm.contactLinks || []).find(item => item.type === 'telegram')?.value || '',
                        contactWhatsapp: (shopForm.contactLinks || []).find(item => item.type === 'whatsapp')?.value || '',
                        contactEmail: (shopForm.contactLinks || []).find(item => item.type === 'email')?.value || '',
                        contactQq: (shopForm.contactLinks || []).find(item => item.type === 'qq')?.value || '',
                        contactWechat: (shopForm.contactLinks || []).find(item => item.type === 'wechat')?.value || '',
                        contactLinks: shopForm.contactLinks || [],
                        featureCard: {
                ...
                const paymentConfig = JSON.stringify({
                    ...
                    notify_ticket_closed: payForm.notifyTicketClosed,
                    ...
                    fuzzy_stock_threshold: payForm.fuzzyStockThreshold,
    ...
    const handlePwdChange = async e => {
        ...
        try {
            const r = await fetch('/api/auth/change-password', {
                method: 'POST',
                headers: { Authorization: `Bearer ${token}`, 'Content-Type': 'application/json' },
                body: JSON.stringify({ oldPassword: pwdForm.currentPassword, newPassword: pwdForm.newPassword })
            })
            const d = await r.json()
            ...
            {activeTab === 'basic' && (
                <div className="ts-section">
                    ...
                    <form onSubmit={handleShopSave} className="ts-form">
                        ...
                        <div className="ts-form-group">
                            <label>{L('SEO 网页描述 (Google 搜索摘要)', 'SEO Page Description')}</label>
                            <textarea
                                placeholder={L('请输入网页的 SEO 描述，有助于搜索引擎收录。建议在 80-150 字之间。', 'Enter the SEO description of your store, which helps search engine indexing. Recommended length: 80-150 characters.')}
                                value={shopForm.seoDescription || ''}
                                onChange={e => setShopForm(f => ({ ...f, seoDescription: e.target.value }))}
                                rows={3}
                                style={{ width: '100%', padding: '10px 12px', border: '1px solid var(--border-color)', borderRadius: 'var(--radius-sm)', background: 'var(--bg-secondary)', color: 'var(--text-primary)', fontSize: '0.9rem', fontFamily: 'inherit', resize: 'vertical' }}
                            />
                            <span className="ts-hint">{L('设置您的商城描述，搜索引擎（如谷歌、百度）会将其展示为搜索结果摘要，避免抓取底部的页脚内容。', 'Set your storefront description. Search engines will display it as the search result snippet instead of scraping footer links.')}</span>
                        </div>
                        <div className="ts-form-group">
                            <label>{L('SEO 网页关键词', 'SEO Keywords')}</label>
                            <input
                                type="text"
                                placeholder={L('用英文逗号分隔，例如: 发卡, 自动发卡, 卡密', 'Separate with commas, e.g. digital goods, automatic delivery')}
                                value={shopForm.seoKeywords || ''}
                                onChange={e => setShopForm(f => ({ ...f, seoKeywords: e.target.value }))}
                            />
                            <span className="ts-hint">{L('为您的商城设置几个核心检索词，有助于搜索引擎定位网站分类。', 'Define core keywords for your storefront to help search engines categorize your website.')}</span>
                        </div>
                        ...
                        {/* 模糊库存显示 */}
                        <div className="ts-form-group" style={{ borderTop: '1px solid var(--border-color)', paddingTop: 18, marginTop: 18 }}>
                            <label>{L('模糊库存显示', 'Fuzzy Stock Display')}</label>
                            <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginTop: 4 }}>
                                <label className="ts-switch" style={{ position: 'relative', display: 'inline-block', width: 42, height: 22, cursor: 'pointer' }}>
                                    <input
                                        type="checkbox"
                                ...
                                <span style={{ color: 'var(--text-secondary)', fontSize: '0.85rem' }}>
                                    {payForm.fuzzyStockEnabled 
                                        ? L('已启用模糊库存显示（前台显示为库存充足/少量库存）', 'Fuzzy stock enabled (displays as In Stock / Low Stock)') 
                                        : L('已禁用模糊库存显示（前台显示精确数字）', 'Fuzzy stock disabled (displays exact numbers)')}
                                </span>
                            </div>
                            <span className="ts-hint" style={{ marginTop: 6, display: 'block' }}>{L('开启后前台商品库存不再显示具体数字，而是模糊提示库存状态。', 'When enabled, storefront product stock will show fuzzy status instead of exact numbers.')}</span>
                        </div>

                        {payForm.fuzzyStockEnabled && (
                            <div className="ts-form-group" style={{ marginTop: 14 }}>
                                <label>{L('库存充足阈值', 'Sufficient Stock Threshold')}</label>
                                <input
                                    type="number"
                                    min="1"
                                    value={payForm.fuzzyStockThreshold}
                                    onChange={e => setPayForm(f => ({ ...f, fuzzyStockThreshold: Math.max(1, parseInt(e.target.value, 10) || 1) }))}
                                    style={{ maxWidth: '160px' }}
                                />
                                <span className="ts-hint">{L('库存大于此数量时前台显示「库存充足」，小于或等于且有货时显示「少量库存」。', 'When stock is greater than this quantity, it shows "In Stock". Otherwise, it shows "Low Stock" when in stock.')}</span>
                            </div>
                        )}

                        {/* 商品销量显示开关 */}
                        <div className="ts-form-group" style={{ borderTop: '1px solid var(--border-color)', paddingTop: 18, marginTop: 18 }}>
                            <label>{L('商品销量显示', 'Product Sales Display')}</label>
                            <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginTop: 4 }}>
                                <label className="ts-switch" style={{ position: 'relative', display: 'inline-block', width: 42, height: 22, cursor: 'pointer' }}>
                                    <input
                                        type="checkbox"
                                ...
                                <span style={{ color: 'var(--text-secondary)', fontSize: '0.85rem' }}>
                                    {payForm.showSalesCount 
                                        ? L('前台展示已售销量数据', 'Sales count displayed on storefront') 
                                        : L('前台隐藏已售销量数据', 'Sales count hidden from storefront')}
                                </span>
                            </div>
                            <span className="ts-hint" style={{ marginTop: 6, display: 'block' }}>{L('关闭后，前台商品网格和商品详情页中将不再显示已售销量（如“已售 0”）。', 'When disabled, sold counts will be hidden globally across the storefront.')}</span>
                        </div>

                        {/* 代理系统 */}
                        <div className="ts-form-group" style={{ borderTop: '1px solid var(--border-color)', paddingTop: 18, marginTop: 18, marginBottom: 18 }}>
                        ...
                        <div className="ts-form-group" style={{ borderTop: '1px solid var(--border-color)', paddingTop: 18, marginTop: 4 }}>
                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 6 }}>
                                <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                                    <label style={{ margin: 0 }}>{L('联系我们设置', 'Contact Us Settings')}</label>
                                    {shopForm.contactEnabled && (
                                        <button type="button" className="ts-pay-config-btn" onClick={() => setContactExpanded(!contactExpanded)}>
                                ...
                                <label className="ts-switch" style={{ position: 'relative', display: 'inline-block', width: 42, height: 22 }}>
                                    <input
                                        type="checkbox"
                                        checked={!!shopForm.contactEnabled}
                                        onChange={e => {
                                            const checked = e.target.checked;
                                            setShopForm(f => ({ ...f, contactEnabled: checked }));
                                            if (checked) {
                                    ...
                                    <span style={{
                                        position: 'absolute', cursor: 'pointer', top: 0, left: 0, right: 0, bottom: 0,
                                        background: shopForm.contactEnabled ? 'var(--primary)' : '#ccc',
                                        borderRadius: 22, transition: '0.2s'
                                    }}>
                                        <span style={{
                                            position: 'absolute', height: 16, width: 16, left: shopForm.contactEnabled ? 23 : 3,
                                            top: 3, background: '#fff', borderRadius: '50%', transition: '0.2s'
                                        }} />
                                    </span>
                                </label>
                            </div>
                            <span className="ts-hint">{L('启用后，将在商城底部的 Footer 区域展示联系我们渠道列表', 'When enabled, shows the Contact Us channels list in the store footer')}</span>

                            {shopForm.contactEnabled && contactExpanded && (
                                <div className="ts-pay-config-wrapper" style={{ marginTop: 14 }}>
                                    <div className="ts-pay-fields" style={{ display: 'flex', flexDirection: 'column', gap: 16, padding: '16px', background: 'var(--bg-tertiary)', borderRadius: 'var(--radius-sm)', border: 'none' }}>
                                        ...
                                        {(shopForm.contactLinks || []).length === 0 ? (
                                            ...
                                            <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                                                {(shopForm.contactLinks || []).map((item, index) => {
                        ...
                        <div style={{ borderTop: '1px solid var(--border-color)', paddingTop: 18, marginTop: 4 }}>
                            ...
                            <div style={{ display: 'flex', flexDirection: 'column', gap: 0 }}>
                                {[
                                    ...
                                    { key: 'notifyTicketClosed', icon: '🎫', label: L('工单已关闭通知', 'Ticket Closed Notification'), desc: L('当客服/管理员关闭工单后自动向买家发送工单关闭提醒邮件', 'Send a ticket closed notification email to buyer when support closes a ticket') },
                                ].map(item => (
